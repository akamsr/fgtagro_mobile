...
class ProductService {
  ...
  Future<Map<String, dynamic>> getPublishedProducts({int page = 1, int limit = 20, String? search}) async {
    ...
    if (response.statusCode == 200 || response.statusCode == 201) {
      final body = response.data is String ? jsonDecode(response.data) : response.data;
      final data = body['data']['data'] as List; // Paginated response usually nests list in data.data
  ...
  Future<Map<String, dynamic>> getProductsByCategory(String categoryId, {int page = 1, int limit = 20}) async {
    final response = await locator<ApiClient>().dio.get(
